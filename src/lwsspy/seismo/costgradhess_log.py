        ...
        for tr in self.data:
            ...
            try:
                ...
                for win, tap in zip(tr.stats.windows, tr.stats.tapers):
                    ...
                    se = np.sum(tap * ws ** 2 * dt)
                    de = np.sum(tap * wo ** 2 * dt)
                    costt += 0.5 * np.log(se/de)
            ...
            try:
                ...
                for win, tap in zip(tr.stats.windows, tr.stats.tapers):
                    ...
                    for _i, _dsdm in enumerate(dsdm):
                        ...
                        gt[_i] += np.sum(wsyn * wdsdm * tap) * dt / se
            ...
            try:
                ...
                for win, tap in zip(tr.stats.windows, tr.stats.tapers):
                    ...
                    for _i, _dsdm_i in enumerate(dsdm):
                        ...
                        gt[_i] += np.sum(wsyn * wdsdm_i * tap) * dt / se

                        for _j, _dsdm_j in enumerate(dsdm):
                            # Get derivate with respect to model parameter j
                            wdsdm_j = _dsdm_j[win.left:win.right]

                            np.sum(wsyn * wdsdm_i * tap) * dt / se

                            A = np.sum(wdsdm_i * wdsdm_j * tap * dt) * se
                            B = np.sum(wsyn * wdsdm_i * tap * dt) \
                                * 2 * np.sum(wsyn * wdsdm_j * tap * dt)
                            ht[_i, _j] += (A-B)/(se**2)
